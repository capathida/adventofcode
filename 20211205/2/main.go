...
func main() {
	...
	for _, r := range lines {
		...
		if arr[0] == arr[2] {
			if arr[1] <= arr[3] {
				for y := arr[1]; y <= arr[3]; y++ {
					//fmt.Println("1num", arr[0], arr[1], arr[2], arr[3], arr[0], y)
					board[y][arr[0]]++
				}
			} else {
				for y := arr[3]; y <= arr[1]; y++ {
					//fmt.Println("n1um", arr[0], arr[1], arr[2], arr[3], arr[0], y)
					board[y][arr[0]]++
				}
			}
		} else if arr[1] == arr[3] {
			if arr[0] <= arr[2] {
				for x := arr[0]; x <= arr[2]; x++ {
					//fmt.Println("num1", arr[0], arr[1], arr[2], arr[3], x, arr[1])
					board[arr[1]][x]++
				}
			} else {
				for x := arr[2]; x <= arr[0]; x++ {
					//fmt.Println("nu1m", arr[0], arr[1], arr[2], arr[3], x, arr[1])
					board[arr[1]][x]++
				}
			}
		}
		if arr[0] != arr[2] && arr[1] != arr[3] {
			if arr[0] <= arr[2] && arr[1] <= arr[3] {
				for q, p := arr[0], arr[1]; q <= arr[2]; p, q = p+1, q+1 {
					//fmt.Println("diag1", arr[0], arr[1], arr[2], arr[3], q, p)
					board[p][q]++
				}
			} else if arr[0] >= arr[2] && arr[1] >= arr[3] {
				for q, p := arr[0], arr[1]; arr[2] <= q; p, q = p-1, q-1 {
					//fmt.Println("diag2", arr[0], arr[1], arr[2], arr[3], q, p)
					board[p][q]++
				}
			} else if arr[0] <= arr[2] && arr[1] >= arr[3] {
				//fmt.Println("sdsdfsdfsdfsf", arr[0], arr[1], arr[2], arr[3])
				for q, p := arr[0], arr[1]; arr[2] >= q; p, q = p-1, q+1 {
					//fmt.Println("diag3", arr[0], arr[1], arr[2], arr[3], q, p)
					board[p][q]++
				}
			} else if arr[0] >= arr[2] && arr[1] <= arr[3] {
				for q, p := arr[0], arr[1]; arr[2] <= q; p, q = p+1, q-1 {
					//fmt.Println("diag4", arr[0], arr[1], arr[2], arr[3], q, p)
					board[p][q]++
				}
			}
		}
